...
export class CameraControls {
  ...
  private onMouseWheel(event: WheelEvent) {
    this.camera.translate(vec3.create(0, 0, event.deltaY * 0.01));
  }
  ...
  private onMouseMove = (event: MouseEvent) => {
    ...
    if (event.buttons === 1) {
      ...
      this.camera.rotateEuler(vec3.create(offset[1], offset[0], 0.0));
    }
  ...
  private onKeyDown = (event: KeyboardEvent) => {
    ...
    this.camera.translate(vec3.create(x, 0, y));
  };
